...
public abstract class EnhancedQuickAdapter<T> extends QuickAdapter<T> {
    ...
    @Override
    protected final void convert(int position, BaseAdapterHelper helper, T item) {
        boolean itemChanged = helper.associatedObject == null || !helper.associatedObject.equals(item);
        helper.associatedObject = item;
        convert(position, helper, item, itemChanged);
    }
     ...
     */
    protected abstract void convert(int position, BaseAdapterHelper helper, T item, boolean itemChanged);
}
